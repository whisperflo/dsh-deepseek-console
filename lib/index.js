...
module.exports = {
  ...
  apply(ctx) {
        ...
        try {
          ...
          try {
            const changelog = await curlOnce({
              method: 'GET', url: UPDATE_CHANGELOG_URL,
              headers: { accept: 'text/plain' }, timeoutSec: 8, label: '更新日志服务'
            })
            if (changelog.status >= 200 && changelog.status < 300) releases = parseChangelog(changelog.text)
          } catch (e) {
            // 更新日志不可用不影响版本更新；UI 会显示一个简短兜底提示。
          }
    ...
    const registerRoutes = () => {
      ...
      route('POST', '/api/deepseek/update', async () => runPluginUpdate())
