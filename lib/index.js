...
module.exports = {
  ...
  apply(ctx) {
    ...
    const registerRoutes = () => {
      ...
      if (!ws) return
      // 清理历史孤儿路由：早期版本未持有 disposer，旧路由不会随插件更新自动移除，
      // 会挡住新版本同路径注册。此清理幂等，只删 /api/deepseek/*。
      try {
        for (const table of [ws.exact, ws.prefixes]) {
          if (!table || typeof table.delete !== 'function') continue
          for (const k of Array.from(table.keys ? table.keys() : [])) {
            if (typeof k === 'string' && k.indexOf('/api/deepseek/') === 0) table.delete(k)
          }
        }
      } catch (e) {
        console.error('deepseek-console: 孤儿路由清理失败（跳过）', e && e.message)
      }
      ...
      const route = (method, path, fn) => {
        try {
          // 通过 ctx.effect 持有 disposer：插件停止/更新时自动移除路由
          ctx.effect(() => {
            try {
              return ws.register({
                kind: 'exact',
              ...
              console.error('deepseek-console: HTTP 路由注册失败（跳过，RPC 不受影响）', e && e.message)
              return () => {}
            }
          })
        } catch (e) {
          console.error('deepseek-console: HTTP 路由 effect 失败', e && e.message)
        }
      }
