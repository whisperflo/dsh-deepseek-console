...
`)
    ...
    function createDeepSeekStore() {
      ...
      return { get: () => state, subscribe, start, stop, poll }
    }
...
`)
    ...
    function FloatingWidget() {
      const ds = useDeepSeek()
      const [pos, setPos] = React.useState(() => clampPos((loadPref() || {}).pos))
      ...
      const panel = h(React.Fragment, null,
        h('div', { className: 't' }, h('span', { className: 'ds-dot ds-dot-' + dot }), h('span', null, 'DeepSeek'), ds.requesting ? h('span', { className: 'ds-badge warn', style: { marginLeft: 'auto' } }, '生成中') : null),
        ...
        h('div', { className: 'dshud-btns' },
          h('button', { className: 'dshud-btn', onClick: () => dsStore.poll() }, '刷新'),
          h('button', { className: 'dshud-btn-p', onClick: openConsole }, '打开控制台 →')))

      const rootStyle = pos ? { left: pos.x, top: pos.y } : { right: 16, bottom: 16 }
