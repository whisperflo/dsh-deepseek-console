...
const ROUTES = {
  ...
  'ds/history': ['GET', '/api/deepseek/history'],
...
.ds-pagination{display:flex;align-items:center;justify-content:flex-end;gap:8px;margin-top:12px;padding-top:10px;border-top:1px solid var(--dsw-alias-border-l1);}
.ds-pagination .count{margin-right:auto;font-size:11.5px;color:var(--dsw-alias-label-tertiary,#999);font-variant-numeric:tabular-nums;}
.ds-pagination .page{font-size:11.5px;color:var(--dsw-alias-label-secondary);font-variant-numeric:tabular-nums;min-width:66px;text-align:center;}
.ds-pagination .ds-btn{height:28px;padding:0 10px;font-size:11.5px;}
...
`)
    ...
    function RecentRequests(props) {
      ...
      const load = React.useCallback(async () => {
        try { setData(await rpc('ds/requests', { status, model, range, limit: PAGE_SIZE, offset: page * PAGE_SIZE })) } catch (e) { setData({ data: { rows: [], total: 0, models: [] } }) }
      }, [status, model, range, page])
      React.useEffect(() => { load() }, [load])
      React.useEffect(() => { setPage(0); setOpen(null) }, [status, model, range])
      ...
      return h('div', null,
        ...
        total > PAGE_SIZE ? h('div', { className: 'ds-pagination' },
          h('span', { className: 'count' }, (page * PAGE_SIZE + 1) + '–' + Math.min((page + 1) * PAGE_SIZE, total) + ' / ' + total + ' 条'),
          h('button', { className: 'ds-btn', disabled: page <= 0, onClick: () => { setPage(page - 1); setOpen(null) } }, '上一页'),
          h('span', { className: 'page' }, (page + 1) + ' / ' + pageCount),
          h('button', { className: 'ds-btn', disabled: page >= pageCount - 1, onClick: () => { setPage(page + 1); setOpen(null) } }, '下一页')) : null)
    }

    // 余额变化：分页加载完整历史，避免概览页随着记录增加而无限变长
    function BalanceHistory() {
      const PAGE_SIZE = 8
      const [page, setPage] = React.useState(0)
      const [data, setData] = React.useState(null)
      const load = React.useCallback(async () => {
        try { setData(await rpc('ds/history', { limit: PAGE_SIZE, offset: page * PAGE_SIZE })) } catch (e) { setData({ data: { entries: [], total: 0 } }) }
      }, [page])
      React.useEffect(() => { load() }, [load])
      ...
      return h('div', null,
        !entries.length ? h('div', { className: 'ds-empty' }, '暂无余额变化记录' + (data ? '' : '…')) :
        h('div', { className: 'ds-list' }, entries.map((e) =>
        ...
        total > PAGE_SIZE ? h('div', { className: 'ds-pagination' },
          h('span', { className: 'count' }, (page * PAGE_SIZE + 1) + '–' + Math.min((page + 1) * PAGE_SIZE, total) + ' / ' + total + ' 条'),
          h('button', { className: 'ds-btn', disabled: page <= 0, onClick: () => setPage(page - 1) }, '上一页'),
          h('span', { className: 'page' }, (page + 1) + ' / ' + pageCount),
          h('button', { className: 'ds-btn', disabled: page >= pageCount - 1, onClick: () => setPage(page + 1) }, '下一页')) : null)
    }
    ...
    function Overview(props) {
      const { account, stats, lastChange, ds, onRefresh, syncing, down, latencyMs, balanceWarning, finance } = props
      ...
      return h('div', { className: 'ds-scope' },
        ...
        h(Panel, { title: '余额变化' }, h(BalanceHistory)))
    }
